...
public class IdempotencyStoreFilter extends OncePerRequestFilter {
    ...
    private boolean isStoreIdempotencyKey(HttpServletRequest request) {
        return Optional.ofNullable(request.getAttribute("storeIdempotencyKey")).filter(Boolean.class::isInstance).map(Boolean.class::cast)
                .orElse(false);
    }

    private Optional<Long> getCommandId(HttpServletRequest request) {
        return Optional.ofNullable(request.getAttribute("commandSourceId")).filter(Long.class::isInstance).map(Long.class::cast);
    }
}
